...
| ASNEXPR '$' {if (!e){$$ = $1;}}
    ...
    '$' M FORINCR 
    {
        ...
        code = incr_start;
        $<addr>$ = incr_start;  /* pass incr_start forward as $11       */
    }
  ')' M {loop_depth++;} A
    {
        if (!e) {
             ...
             if (!e){
                 ...
                 if (b1_T != NULL && b1_T->addr == FALL_THROUGH
                     ...
                     if (flipCondToTrue(b1_F->addr)) {
                         ...
                         b1_F = $1->T;  /* FALL_THROUGH → false falls into B2 */
                     }
                 }
                 backpatch(b1_F, $3);
                 $$->T = merge(b1_T, $4->T);
...
void generateTACFlowDOT() {
    ...
    for (int i = 0; i < code; i++) {
        ...
        if (!in_func && strstr(imcode[i], "BeginFunc") == NULL) {
            ...
            // Sequential flow inside global scope
            ...
            if (is_call_g) {
                ...
                if (i + 1 < code && strstr(imcode[i+1], "BeginFunc") == NULL)
                    ...
                    fprintf(dot, "  }\n"); // close global cluster when next is BeginFunc or end
            }
            continue;
        }
...
void identifyBasicBlocks() {
    ...
    for (int i = 0; i < code; i++) {
        ...
        // BeginFunc: itself is a leader, and next instruction is a new leader
        if (strstr(imcode[i], "BeginFunc") != NULL) {
            is_leader[i] = 1;
            if (i + 1 < code) is_leader[i + 1] = 1;
        }
        // EndFunc: itself starts a new block (solo), next is also a leader
        if (strstr(imcode[i], "EndFunc") != NULL) {
            is_leader[i] = 1;
            if (i + 1 < code) is_leader[i + 1] = 1;
        }
        // Return ends a block — next instruction is a new leader (EndFunc)
...
void generateTACFlowWithBlocks() {
    ...
    // Blocks outside any function are global-scope blocks — show them labeled accordingly
    BasicBlock* bb = blocks;
    while (bb) {
        // Determine if this block is inside a function or global scope
        int is_global = 1;
        ...
        // Check if any line in block is inside a BeginFunc..EndFunc span
        {
            int inside = 0;
            for (int i = 0; i < code; i++) {
    ...
    // --- Pass 1: call/return edges for EVERY Call instruction in any block ---
    // We cannot restrict to block-end only, because multiple Calls can be
    // bundled inside the same block (e.g. Block 4 has 6 x Call hello).
    // Each Call site must show: call edge -> callee BeginFunc block
    //                           ret  edge <- callee EndFunc block -> next instruction's block
    // To avoid duplicate edges (same src->dst pair emitted multiple times)
    // we track which (src_block, dst_block) pairs have already been drawn.
    ...
    for (int i = 0; i < code; i++) {
        ...
        // call edge: src_block -> callee BeginFunc block
        if (callee_begin >= 0) {
        ...
        // ret edge: callee EndFunc block -> block containing instruction after this call
        if (callee_end >= 0 && i + 1 < code) {
    ...
    // --- Pass 2: sequential / goto / return edges (block-end instructions only) ---
    for (int i = 0; i < code; i++) {

        int current_block = getBlockForLine(i);
        if (current_block == -1) continue;

        // Only process the last instruction of each block
        bb = blocks;
        int is_block_end = 0;
        while (bb) {
        ...
        if (is_conditional) {
            ...
            // Sequential flow after last call in block (call/ret edges drawn in Pass 1)
            if (i + 1 < code && strstr(imcode[i+1], "BeginFunc") == NULL
                              && strstr(imcode[i+1], "EndFunc")   == NULL) {
                int next_block = getBlockForLine(i + 1);
                if (next_block != -1 && next_block != current_block)
...
void generateInteractiveDashboard() {
   ...
   // fprintf(index, "<div class=\"nav-grid\">\n");
    fprintf(index, "<div class=\"nav-grid\" style=\"display:grid;grid-template-columns:repeat(3,1fr);gap:30px;margin-top:20px;\">\n");
...
// Generate TAC page with toggle between optimized and unoptimized code
void generateTACPage() {
    ...
    // Add inline styles for toggle switch
    fprintf(html, "<style>\n");
...
fprintf(html, "</div>\n");
    ...
    // Code viewer with ID for JavaScript manipulation
    ...
    fprintf(html, "        }\n");
    fprintf(html, "        const optResponse = await fetch('opt.tac');\n");
