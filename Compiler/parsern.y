...
static int licm_is_dead(int i) {
    ...
    /* Dead if it is an unfilled preheader slot */
    if (strstr(imcode[i], "LICM_PREHEADER_SLOT") != NULL) return 1;
    /* Lines with "// LICM:" are either:
     *   (a) in-loop NOPs:  "21 // LICM: found hoisted ..."  -- NO '=' before the tag
     *   (b) live preheader lines written by a PREVIOUS hoisting round that
     *       had comments.  We now write preheader lines without comments (Bug 2 fix),
     *       but guard here too: only treat as dead when there is NO '=' before the tag.
     */
    const char *tag = strstr(imcode[i], "// LICM:");
    if (tag) {
        const char *eq = strchr(imcode[i], '=');
        if (!eq || eq > tag) return 1;   /* no real assignment before the tag -> NOP */
        /* There IS an '=' before the tag -> live hoisted instruction, not dead */
    }
    return 0;
}
...
void loopInvariantCodeMotion() {
     ...
     * Rule: if Hb is inside (Ha..Ba], extend Ba = max(Ba,Bb). Repeat. */
    {
        ...
        /* Remove loops fully subsumed: H deeper but same B */
    ...
    for (int li = 0; li < nloops; li++) {
        ...
        for (int I = H+1; I <= B && ncands < 200; I++) {
            ...
            // Check 5: dominance
            // A conditional branch "if cond goto bt" at line k means:
            //   TRUE  path -> jumps to bt
            //   FALSE path -> falls through to k+1
            // If bt >= I: the TRUE path jumps to I or past it, meaning the FALSE
            // path (fallthrough k+1) may reach an unconditional goto that exits
            // the loop body BEFORE ever reaching I — so I is NOT guaranteed to
            // execute on every loop iteration.
            // Example: "if arr[t1]==target goto 20"  (I=20)
            //   TRUE:  goes to line 20 (found=1) — executes it
            //   FALSE: falls to "goto 14" — SKIPS line 20 entirely
            // With the old check (bt > I), bt==I passes. Fix: bt >= I.
            int dominated = 1;
            for (int k=H; k<I; k++) {
                if (!body[k]||licm_is_dead(k)) continue;
        ...
        for (int c = 0; c < ncands; c++) {
            ...
            /* Write CLEAN instruction to preheader slot — no trailing comment.
             * The TAC->assembly generator parses output.tac line by line and
             * a "// LICM:" comment on the same line as a real instruction
             * causes it to misparse or skip the instruction entirely. */
            if (is_binary)
            ...
            /* In-loop NOP: pure comment — safe to annotate, assembly gen skips it */
...
| ASNEXPR '$' {if (!e){$$ = $1;}}
        ...
        | IF '(' BOOLEXPR ')' M A {
    if (!e) {
        /* "if not cond goto F": true body at $5 (falls through), F = skip body */
        /* F list should point to after the body, which is the next statement = $$->N */
        /* We don't know that yet — so F becomes part of N (all go to "next") */
        $$ = createBoolNode();
        $$->N = merge($3->F, $6->N);  /* both "skipped" and "body fell through" go to next */
        $$->B = $6->B;
        $$->C = $6->C;
    }
}
 |IF '(' BOOLEXPR ')' M A ELSE NN M A {
    if (!e) {
        /* "if not cond goto F": true body at $5 (falls through), F → else body at $9 */
        backpatch($3->F, $9);   /* false: jump to else body */
        /* T is NULL — true falls through to $5 naturally, no backpatch needed */
        $$ = createBoolNode();
    ...
    backpatch($8->N, $2);
    backpatch($8->C, $2);
    /* "if not cond goto F" — F list jumps over body, so backpatch F to the code AFTER the back-edge goto */
    /* true (body) falls through to $6 naturally */
    /* We emit the back-edge goto first, THEN backpatch F to code (= after the goto) */
    sprintf(imcode[code],"%d goto %d\n", code, $2);
    code++;
    backpatch($4->F, code);   /* exit loop: go to here, past the back-edge goto */
    backpatch($8->B, code);
    $$ = createBoolNode();
    $$->N = NULL;             /* while loop itself has no fall-through next list */
}}
    ...
    if (!e) {
        ...
        backpatch($4->N, $6);
        backpatch($4->C, $6);
        /* "if not cond goto F": T=NULL(falls through=loop again back to $2 via goto), F=exit */
        /* We need: if condition true → go back to $2 */
        /* But T is NULL (fall-through after the condition instruction).
           So we emit an unconditional goto $2, then backpatch F past it. */
        sprintf(imcode[code], "%d goto %d\n", code, $2);
        code++;
        backpatch($8->F, code);   /* exit: past the goto */
        backpatch($4->B, code);
        $$ = createBoolNode();
        $$->N = NULL;
    }
  ...
  ')' M {loop_depth++;} A 
    {
        if (!e) {
            loop_depth--;
            /* "if not cond goto F": true(body) falls through to $12, F(exit) to be backpatched */
            /* body is at $12 (falls through from condition when true) */
            /* We must NOT backpatch F to $12 — that would send false into the body! */
            /* F = exit, patch it to after the final back-edge goto */
            backpatch($14->N, $8);
            backpatch($14->C, $8);
            sprintf(imcode[code], "%d goto %d\n", code, $8);
            code++;
            backpatch($6->F, code);   /* exit: past final back-edge goto */
            backpatch($14->B, code);
            $$ = createBoolNode();
            $$->N = NULL;
        }
...
BOOLEXPR:
         BOOLEXPR OR M BOOLEXPR {
             /* OR short-circuit with if-not-goto-F leaves:
                T=NULL means "true = fall through". For OR, left-true must SKIP the right side.
                Since T is NULL (fall-through), we must emit a goto hole BEFORE the M marker
                captures the position of the right side.
                The grammar is: BOOLEXPR(1) OR M(3) BOOLEXPR(4)
                M fires AFTER OR is seen, so $3 = address of right side's first instruction.
                At that point left has already been fully parsed and its fall-through already
                ran into right. We can't insert a goto retroactively before right.
                Solution: emit the goto-skip-right hole INLINE inside the OR action,
                targeting "after right" — add it to T-list to be resolved later.
                But "after right" isn't known yet. So we keep it in $$->T (unresolved T hole).
                backpatch($1->F, $3): left-false → try right. */
             if (!e){
                 backpatch($1->F, $3);
                 $$ = createBoolNode();
                 $$->T = merge($1->T, $4->T);
    ...
    | BOOLEXPR AND M BOOLEXPR {
        /* AND short-circuit with if-not-goto-F leaves:
           T=NULL means true=fall-through. Left-true falls into $3 (right side). Correct.
           Left-false ($1->F) = whole AND is false, merges into result F. */
        if (!e){
            $$ = createBoolNode();
            $$->T = $4->T;
            $$->F = merge($1->F, $4->F);
        ...
        | EXPR LT EXPR {
       if(!e) {
           sprintf(imcode[code], "%d if not %s %s %s goto ", code, $1->str, $2, $3->str);
           $$ = createBoolNode();
           $$->F = createNode(code); code++;
           $$->T = NULL;   /* true falls through */
       }
   }
   | EXPR GT EXPR {
       if(!e) {
